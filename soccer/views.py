...
from .forms import PlayerForm, GameCountryForm,DeleteForm
...
def Update(request,team_number):
    ...
    return render(request, 'soccer/update.html', {'formset': RequestFormset,'teamunumber':teamnumber})



@login_required()
def DeleteView(request,team_number):
    form = DeleteForm
    if request.method == 'POST':
        Delete_queryset = Player.objects.filter(teamnumber=int(team_number)).filter(author=request.user)
        MyTeamNumber_obj = MyTeamNumber.objects.filter(my_team_number = team_number).filter(author=request.user)
        MyTeamNumber_obj.delete()
        for object in Delete_queryset:
            object.delete()
        return redirect(to='/make')


    return render (request, 'soccer/delete.html', {'form':form})
...
def GameView(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'soccer/game.html', {'form':form,'No_team_coment':No_team_coment})